...
> Обратите внимание, метод `Replace` имеет перегруженную версию, принимающую делегат `MatchEvaluator`, который вызывается для каждого совпадения. Это позволяет пору­чить построение содержимого строки замены коду С#, если язык регулярных выраже­ний в такой ситуации оказывается недостаточно выразительным.
