...
![](\pastes\2022-02-27-13-57-09.png)  
Рис. 1. Составляющие элементы процесса работы планировщика задач

Организация параллельного приложения с помощью пула потоков позволяет избежать накладных расходов, связанных с созданием, запуском и завершением потоков для каждого фрагмента в программе, допускающего распараллеливание.
...
```
Итак, получаем, что задача `Task` выполняется в пуле, методы `Parallel.Invoke`, `Parallel.For` неявно создают задачи, которые тоже выполняются в пуле. Метод `Main` и пользовательский поток не являются рабочими потоками пула. Часть работы, определяемой в методах` Parallel.Invoke`, `Parallel.For`, может выполнять в основном потоке.
...
![](\pastes\2022-02-27-14-40-47.png)  
...
![](\pastes\2022-02-27-15-50-41.png)  
