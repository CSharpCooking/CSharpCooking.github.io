...
В целях демонстрации предположим, что имеется окно WPF с текстовым полем по имени _txtMessage_, содержимое которого должно быть обновлено рабочим пото­ком после выполнения длительной задачи (эмулируемой с помощью вызова метода _Thread.Sleep_). Ниже приведен необходимый код:
...
Табл. Сводное описание реализаций _SynchronizationContext_
<style type="text/css">
