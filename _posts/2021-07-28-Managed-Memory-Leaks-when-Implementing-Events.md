...
---
В посте показано, каким образом события могут вызывать утечки управляемой памяти. Предлагаются два решения, чтобы избежать подобных утечек. Первое решение связано с реализацией метода `Dispose` для отмены подписки, во втором решении демонстрируется применение слабых ссылок. См. <a href="https://csharpcooking.github.io/2021/07/28/Managed-Memory-Leaks-when-Implementing-Events.html" style="text-decoration: underline; font-weight: bold;">далее</a>.
