# Ответы к тесту №9 видеокурса «Архитектура ЭВМ и основы ОС» 
...
- Предоставление сервисов: в ранних версиях операционных систем, BIOS также предоставлял набор базовых сервисов ввода-вывода для взаимодействия с аппаратными компонентами, такими как клавиатура, экран и дисковые устройства.
...
- Совместимость с файловыми системами:
  ...
  - GRUB: Поддерживает широкий диапазон файловых систем, включая ext2, ext3, ext4, FAT, NTFS, и многие другие.
...
Во время выполнения кода BIOS, процессор работает в режиме реального адресации, часто называемом real mode. Этот режим является первоначальным режимом работы x86-процессоров, который использовался в оригинальных процессорах Intel 8086 и 8088.
...
- 
  Многоядерность: GRUB позволяет выбрать, какую операционную систему загрузить при старте компьютера, предоставляя пользователю меню с настройками загрузки.

- 
  Гибкость: GRUB предоставляет гибкие опции конфигурации, позволяя изменять параметры загрузки и даже вводить команды во время процесса загрузки.

- 
  Поддержка файловых систем: GRUB поддерживает широкий диапазон файловых систем, что позволяет загружать ядро операционной системы и другие файлы с различных типов разделов.

- 
  Поддержка сетевой загрузки: GRUB может быть настроен для загрузки операционной системы через сеть, используя протоколы, такие как TFTP.

- 
  Поддержка (U)EFI и BIOS: GRUB может работать с традиционным BIOS, а также с более современным интерфейсом (U)EFI.

- 
  Восстановление и отладка: GRUB обеспечивает функции восстановления, что может быть полезно при решении проблем с загрузкой системы.
...
- 
  Bootstrap Code (загрузочный код): занимает 446 байтов и содержит исполняемый код, который выполняется при запуске компьютера. Этот код часто называют загрузочным загрузчиком (bootloader) и его задача – найти и запустить операционную систему.
...
UEFI – это современный интерфейс между операционной системой и платформенным прошивочным ПО, и он предоставляет множество улучшений по сравнению с традиционным BIOS. Одним из ключевых улучшений, которые предоставляет UEFI, является поддержка GPT.
