...
```csc Program.es /define:TESTMODE,PLAYMODE```
...
Утверждение представляет собой то, что в случае нарушения говорит об ошибке в коде текущего метода. Для сравнения: генерация исключения на основе проверки достоверности аргумента указывает на ошибку в коде вызывающего компонента.
